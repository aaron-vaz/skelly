...
// Version can be injected at build time using -ldflags "-X github.com/aaron-vaz/proj/internal/cli.version=x.y.z"
var version = "dev"
...
func (i *FlagCommandInvoker) Execute(args []string) error {
	...
	switch args[1] {
	case cmdVersion:
		return i.ui.RenderInfo(fmt.Sprintf("proj version %s", version))
	case cmdInit:
...
func (i *FlagCommandInvoker) showHelp(command string) {
	...
	case cmdInit:
		fmt.Println("USAGE:")
		fmt.Println("    proj init --src SOURCE [--dst DEST]")
		fmt.Println("")
		fmt.Println("ARGS:")
		...
		fmt.Println("    $ proj init --src https://github.com/user/template")
		fmt.Println("    $ proj init --src https://github.com/user/template --dst ./my-project")
	default:
		fmt.Println("USAGE:")
		fmt.Println("    proj COMMAND [OPTIONS]")
		fmt.Println("")
		fmt.Println("COMMANDS:")
		fmt.Println("    init        Initialize a new project from a template")
		fmt.Println("    help        Show command help")
		fmt.Println("    version     Show proj version")
		fmt.Println("")
		fmt.Println("OPTIONS:")
		...
		fmt.Println("")
		fmt.Println("Run 'proj help COMMAND' for command-specific help")
	}
}
